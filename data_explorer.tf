...
module "kusto_attached_database_configurations" {
  ...
  cluster_id          = can(each.value.cluster_resource_id) ? each.value.cluster_resource_id : local.combined_objects_kusto_clusters[try(each.value.kusto_cluster.source.lz_key, local.client_config.landingzone_key)][each.value.kusto_cluster.source.key].id
