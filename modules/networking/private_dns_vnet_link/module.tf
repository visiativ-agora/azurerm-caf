...
resource "azurerm_private_dns_zone_virtual_network_link" "vnet_links" {
  ...
  private_dns_zone_id  = can(each.value.private_dns_zone_id) ? each.value.private_dns_zone_id : var.private_dns[try(each.value.lz_key, var.client_config.landingzone_key)][each.value.key].id
