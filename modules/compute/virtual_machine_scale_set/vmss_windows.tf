...
resource "azurerm_windows_virtual_machine_scale_set" "vmss" {
  ...
  dynamic "data_disk" {
    ...
    content {
      ...
      disk_iops_read_write      = contains(["PremiumV2_LRS", "UltraSSD_LRS"], data_disk.value.storage_account_type) ? try(data_disk.value.disk_iops_read_write, data_disk.value.ultra_ssd_disk_iops_read_write, null) : null
      disk_mbps_read_write      = contains(["PremiumV2_LRS", "UltraSSD_LRS"], data_disk.value.storage_account_type) ? try(data_disk.value.disk_mbps_read_write, data_disk.value.ultra_ssd_disk_mbps_read_write, null) : null
  ...
  dynamic "automatic_os_upgrade_policy" {
    ...
    content {
      automatic_rollback_enabled   = try(each.value.automatic_os_upgrade_policy.automatic_rollback_enabled, each.value.automatic_os_upgrade_policy.disable_automatic_rollback == false)
      automatic_os_upgrade_enabled = try(each.value.automatic_os_upgrade_policy.automatic_os_upgrade_enabled, each.value.automatic_os_upgrade_policy.enable_automatic_os_upgrade)
    }
  }
